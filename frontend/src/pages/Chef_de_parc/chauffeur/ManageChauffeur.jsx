...
import { Search, Edit, UserX, UserCheck, Eye, Save, X, Users } from 'lucide-react';
...
const ManageChauffeur = () => {
  ...
  const handleParcChange = (parc) => {
    ...
    setSearchTerm(''); // Reset search when changing parc
  };

  // Fonction pour obtenir les chauffeurs par parc
      ...
      3: [ // Parc Provincial Fès
        {
          ...
          numero_permis: 'P321654987',
          type_permis: 'B',
          date_expiration_permis: '2025-09-15',
          nombre_points: 11,
          est_actif: true
        }
      ],
      4: [ // Parc Local Marrakech
        {
          id: 6,
          matricule: 'CH006',
          nom: 'Tazi',
  ...
  // Charger les chauffeurs quand un parc est sélectionné
  useEffect(() => {
    if (selectedParc) {
      const chauffeursParc = getChauffeursParParc(selectedParc.id);
      setChauffeurs(chauffeursParc);
      setFilteredChauffeurs(chauffeursParc);
    }
  }, [selectedParc]);

  // Données simulées des chauffeurs (ancien code commenté)
  /*useEffect(() => {
  ...
  return (
    <Layout title="Gérer les Chauffeurs" userType="chef de parc">
      <div className="p-6">
        {/* Sélecteur de parc */}
        <ParcSelector 
        ...
        {selectedParc && (
          <>
            <div className="mb-6 mt-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Gérer les Chauffeurs - {selectedParc.nom}</h1>
        ...
        {!selectedParc && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">Veuillez sélectionner un parc pour gérer les chauffeurs</p>
          </div>
        )}
