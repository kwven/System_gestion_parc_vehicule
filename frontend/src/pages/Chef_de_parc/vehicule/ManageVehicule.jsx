...
const ManageVehicule = ({ userType }) => {
  ...
  // Données simulées des véhicules par parc
  const getVehiculesParParc = (parcId) => {
    const allVehicules = {
      1: [ // Parc Central Rabat
        {
          id: 1,
          immatriculation: '123-RAB-456',
          marque: 'Toyota',
          modele: 'Corolla',
          type: 'Berline',
    ...
    return allVehicules[parcId] || [];
  };

  // Charger les véhicules quand un parc est sélectionné
  useEffect(() => {
    if (selectedParc) {
      const vehiculesParc = getVehiculesParParc(selectedParc.id);
      setVehicules(vehiculesParc);
      setFilteredVehicules(vehiculesParc);
    }
  }, [selectedParc]);
  ...
  return (
    <Layout title="Gestion des véhicules" userType="chef de parc">
      <div className="p-6">
        {/* Sélection du parc */}
        <ParcSelector 
        ...
        {selectedParc && (
          <>
            <div className="mb-6 mt-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Gérer les Véhicules - {selectedParc.nom}</h1>
