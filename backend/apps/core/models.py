...
class VehiculeParc(models.Model):
    ...
    parc = models.ForeignKey(Parc,on_delete=models.CASCADE,null=False,blank=False,verbose_name="Parc")
    date_affectation = models.DateField(default=timezone.now,null=False,blank=False,verbose_name="Date de début de l'affectation du véhicule au parc")
    ...
    def __str__(self):
        return f"{self.vehicule} rattaché à {self.parc} depuis {self.date_affectation}"
    def clean(self):
        super().clean()
        if self.date_retrait and self.date_affectation and self.date_retrait < self.date_affectation:
            raise ValidationError({'date_retrait': "La date de retrait ne peut pas être antérieure à la date d'affectation."})
